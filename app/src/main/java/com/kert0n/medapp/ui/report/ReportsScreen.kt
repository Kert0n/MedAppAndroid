...
/**
 * Три вкладки различает **время**, и порядок у них — ось времени слева направо: «Принято» —
 * что уже принято за период, «Сводка» — что лежит сейчас, «Расход» — сколько уйдёт до выбранной
 * даты. Прежний порядок шёл настоящее → будущее → прошлое, и взгляд, уйдя вперёд, возвращался
 * назад (решение владельца).
 *
 * Порядок вкладок задаётся здесь: экран рисует их перечислением. Вкладка по умолчанию от порядка
 * не зависит — оболочка называет `SUMMARY` прямо, а сохранённое состояние держит имя, а не номер.
 */
enum class ReportsMode { SPENT, SUMMARY, FUTURE }
