...
data class OutstandingOperation(
    ...
    enum class Trouble {
        /**
         * Ждёт отправки или ответа: делать нечего, и действий у строки нет. Сюда же попадает
         * строка, которой не хватило словаря, — её дочитает работник, но незакрыта она так же,
         * как всякая другая, и прятать её значило бы сказать «всё доехало» раньше времени.
         */
        WAITING,
