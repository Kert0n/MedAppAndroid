...
class CourseDraft(
     ...
     * отвергается: правило «лечение без единой дозы — не лечение» живёт на [Prescription], и такой
     * черновик просто не активируется. Выделения идут следом: приёмов стало меньше — лишнее снимается с
     * конца, потому что держать под лечение больше, чем ему нужно, незачем (PLAN D5).
     *
     * Зажим стоит **здесь**, а не в сценарии: число приёмов и состав лежат в одной записи, и
    ...
    /**
     * Сколько потребность оставляет пачке сверх выделенного остальным; `null` — число приёмов не
     * названо, и ограничивать нечем: состав собирают и до того, как врач назовёт длительность.
     * Коробки здесь ни при чём — черновик их не читал.
     */
    fun needLeftFor(pkg: PackageRef): Doses? = totalDoses?.let { medicine.needLeftFor(pkg, it) }
