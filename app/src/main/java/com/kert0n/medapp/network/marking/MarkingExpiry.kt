 ...
 * приходит моментом в миллисекундах, и та же полночь по Москве в UTC — ещё вчера. Реестр маркировки —
 * российский реестр, и его сутки — московские. Момент до нуля и нечитаемая дата — срока нет.
 */
internal val MARKING_ZONE: ZoneId = ZoneId.of("Europe/Moscow")

internal fun MarkingCheckNetworkDTO.expiresOn(): ExpiryDate? {
    val millis = expireDate?.takeIf { it > 0 } ?: return null
    return attempt { ExpiryDate(Instant.ofEpochMilli(millis).atZone(MARKING_ZONE).toLocalDate()) }.getOrNull()
}
 ...
 */
internal fun MarkingCheckNetworkDTO.boughtOn(today: LocalDate): LocalDate? {
    val millis = receiptDate?.takeIf { it > 0 } ?: return null
    val day = attempt { Instant.ofEpochMilli(millis).atZone(MARKING_ZONE).toLocalDate() }.getOrNull()
