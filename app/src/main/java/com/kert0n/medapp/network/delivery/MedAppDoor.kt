...
/**
 * Курьер ходит к серверу через тот же клиент, что и всё остальное: пропуск, лог, таймауты — и
 * механический повтор: перевыпуск пропуска по `401` и один повтор уже сделаны в [MedAppApi].
 */
class MedAppDoor @Inject constructor(private val api: MedAppApi) : CourierDoor {
