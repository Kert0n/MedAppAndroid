...
/**
 * Один вопрос реестру маркировки: код уходит байт в байт под литеральным `{FNC1}`, без пропуска
 * MedApp; «не найдено» — обычный ответ, а не ошибка; чужая форма ответа — не провал (PLAN H5, G3).
 */
class MarkingApiTest {

    private val requests = mutableListOf<HttpRequestData>()

    private fun api(answer: MockRequestHandleScope.(HttpRequestData) -> HttpResponseData): MarkingApi = MarkingApi(
        markingHttpClient(
            MockEngine { request ->
                requests += request
                answer(request)
    ...
    /** Адрес проверки приходит из сборки целиком — запрос уходит ровно на него. */
    private val CHECK_URL = "https://marking.test/check"
    ...
    private val code = DataMatrixCode(MarkingFixtures.SCANNED)
    ...
    fun theRequestCarriesTheCodeVerbatimUnderALiteralFnc1() = runTest {
        api { json(MarkingFixtures.found) }.check(code)

        val request = requests.single()
        assertEquals(HttpMethod.Post, request.method)
        assertEquals(CHECK_URL, request.url.toString())
        val body = Json.parseToJsonElement((request.body as TextContent).text).jsonObject
        val sent = body.getValue("code").jsonPrimitive.content
        assertEquals("{FNC1}" + MarkingFixtures.SCANNED, sent)
        assertTrue("разделители GS внутри кода должны остаться", sent.contains(MarkingFixtures.GS + "91"))
    ...
    fun aFoundCodeYieldsTheBody() = runTest {
        val check = api { json(MarkingFixtures.found) }.check(code)

        val body = (check as MarkingCheck.Body).dto
    ...
    fun notFoundIsAnAnswerNotAnError() = runTest {
        assertEquals(MarkingCheck.NotFound, api { json(MarkingFixtures.notFound) }.check(code))
        assertEquals(MarkingCheck.NotFound, api { respond("", HttpStatusCode.NotFound) }.check(code))
        assertEquals(MarkingCheck.NotFound, api { respond("", HttpStatusCode.BadRequest) }.check(code))
    }

    @Test
    fun aBrokenConnectionIsNoConnection() = runTest {
        assertEquals(MarkingCheck.Unavailable(Unavailability.NO_CONNECTION), api { throw ConnectException("связи нет") }.check(code))
    }
    ...
    fun aLegalOrPolicyRefusalIsARefusalNotSilence() = runTest {
        assertEquals(MarkingCheck.Unavailable(Unavailability.SERVER_REFUSED_US), api { respond("", MarkingApi.UNAVAILABLE_FOR_LEGAL_REASONS) }.check(code))
        assertEquals(MarkingCheck.Unavailable(Unavailability.SERVER_REFUSED_US), api { respond("", HttpStatusCode.Forbidden) }.check(code))
    }

    @Test
    fun anythingElseIsAServerThatKeptSilent() = runTest {
        assertEquals(MarkingCheck.Unavailable(Unavailability.SERVER_SILENT), api { respond("", HttpStatusCode.BadGateway) }.check(code))
        assertEquals(
            MarkingCheck.Unavailable(Unavailability.SERVER_SILENT),
