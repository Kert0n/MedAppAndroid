...
class SyncCommandStorageConverterTest {
    ...
    private val everyKind: List<SyncCommand> = listOf(
        ...
        PackageSyncCommand.Create(PACK, HOME_KIT, fromMedKitId = SHARED_KIT),
    ...
    fun theShelfABoxCameFromSurvivesTheRoundTrip() {
        val carried = PackageSyncCommand.Create(PACK, HOME_KIT, fromMedKitId = SHARED_KIT)
        assertEquals(SHARED_KIT, (roundTrip(carried) as PackageSyncCommand.Create).fromMedKitId)
        assertNull((roundTrip(PackageSyncCommand.Create(PACK, HOME_KIT)) as PackageSyncCommand.Create).fromMedKitId)
    }
