...
/**
 * **История Кати** (PLAN U1 «история человека», `docs/истории.md`).
 *
 * Катя лечится десять дней, дважды в день по две таблетки, и в начале всё сходилось: в коробке было
 * двадцать таблеток — ровно на десять приёмов, и приложение обещало, что лечения хватит до конца.
 * Она на это и рассчитывала: докупать ничего не собиралась.
 *
 * На третий день она открыла коробку и увидела, что таблеток там меньше, чем она думала: часть
 * выпила дочь от головной боли, никому не сказав. Катя пересчитывает коробку — шесть таблеток
 * вместо четырнадцати, — и с этой минуты лечение **не обеспечено**: приложение обязано сказать это
 * прямо, числом и днём, а не промолчать и не перестроить расписание за неё.
 *
 * Здесь и была бы самая обидная беда: молчащее приложение оставило бы Катю уверенной, что до конца
 * лечения всё есть, и она узнала бы правду в тот вечер, когда таблетка кончилась. Поэтому история
 * идёт до карточки лечения и читает то, что там написано.
 *
 * Расписание при этом не трогается: сколько и когда принимать, решил врач, а нехватка — это про
 * запас, а не про назначение (PLAN C1, D5).
 */
@HiltAndroidTest
