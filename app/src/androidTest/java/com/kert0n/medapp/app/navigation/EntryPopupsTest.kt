...
class EntryPopupsTest {
    ...
    /**
     * **Сначала срок, потом пропуски — по одному окну.** Срок кончается сегодня, и его решать сейчас;
     * пропуск ждёт соседнего окна. Крестик срока открывает попап пропущенного, его крестик — ничего.
     */
    @Test
    fun theExpiryNewsComesFirstAndTheMissedIntakesAfterIt() {
    ...
    /**
     * **Уход с попапа пропущенного на карточку пункта его не закрывает**, и попап срока, уже
     * закрытый, не возвращается вместо него: вернувшись, человек видит тот же разговор.
     */
    @Test
